...
public class TablePlayerComputer extends JPanel {
	...
	public TablePlayerComputer(int m, int n, int depth, int level) {
				...
				if (blue_player_turn) {
					for (int i = 0; i < lines.size(); i++) {
						if (!isClicked.get(i) && lines.get(i).contains(e.getX(), e.getY())) {
							...
							linesList.get(i).setClicked(true);
							boolean play_again = good_move(i);

							boolean game_over = true;

							for (int j = 0; j < isColored.size(); j++) {
								if (isColored.get(j) == 0) {
									game_over = false;
									break;
								}
							}
	...
	private void computer_move() {
		...
		while (play_again) {
			...
			/*
			 * if (play_again) { str.append(linesList.get(move).getName() +
			 * "//drugi igrac povlaci liniju, formira kvadrat i igra ponovo\n"); } else { if
			 * (move != -1) str.append(linesList.get(move).getName() +
			 * "//drugi igrac povlaci liniju\n"); }
			 */
	...
	public static void main(String[] args) {
		...
		frame.add(new TablePlayerComputer(4, 4, 4, 2), BorderLayout.CENTER);
